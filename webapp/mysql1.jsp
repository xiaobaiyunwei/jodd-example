...
<body>
	...
	{     
		...
		LogManager lmng = new LogManager("D:\\data1_jianshe99_\\", true);  
			...
			String pbase_sql="select postid,content from posts_js01 where siteid=4 and deltag=0 "
				...
				for(int i=0;i<=20;i++){					
					...
					while(rs.next())
					{
						...
						//String sr=HttpRequest.sendPost("http://filter.chnedu.com/retstr", "content="+encode_content);						
						//if(!sr.trim().equals("0")){										
							lmng.log(rs.getInt("postid")+"	http://bbs.jianshe99.com/backend/check_posts_filter.php?postid="+rs.getInt("postid")+"	", true);							
						//}
	...
	out.println("建设网第"+batch_num+"批数据执行完毕！");
	%>
</body>
</html>