...
<body>
	...
	{     
		...
		LogManager lmng = new LogManager("D:\\data2_jianshe99_\\", true);  
			...
			String pbase_sql="select postid,content from posts_js02 where  siteid=4 and deltag=0 "
				...
				for(int i=0;i<=520;i++){
					...
					while(rs.next())
					{
						...
						//String sr=HttpRequest.sendPost("http://filter.chnedu.com/retstr", "content="+encontent);						
						//if(!sr.trim().equals("0")){		
							lmng.log(rs.getInt("postid")+"	http://bbs.chinatat.com/backend/check_posts_filter.php?postid="+rs.getInt("postid"), true);
						//}
					}									
					conn.commit();
