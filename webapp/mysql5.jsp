...
<body>
	...
	{     
		...
		String url="jdbc:mysql://192.168.190.**:3306/wold?rewriteBatchedStatements=true";
		String user="bbs";
		...
		LogManager lmng = new LogManager("F:\\data5", true);  
		
		if (stmt_type==1)
		{ 
			String pbase_sql="select * from posts where postid>=20400115 and postid<23400115 and  deltag=0 "
				...
				for(int i=0;i<=4000;i++){
					...
					conn.commit();
					System.out.println("正在执行mysql5第"+i*batch+"条记录！");
				...
				for(int i=1;i<=5000000;i++){
					...
					if(i%1000==0){
						stmt.executeBatch();
						conn.commit();
						System.out.println("正在执行mysql5第"+i+"条记录！");
					}
				}		
				stmt.close();
		}		
